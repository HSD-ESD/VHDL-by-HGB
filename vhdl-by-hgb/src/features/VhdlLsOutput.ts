    ...
    public async GetOutput(filePath: string) {
        ...
        for (let i = 0; i < documentSymbols.length; i++) {
            ...
            */
            ...
            const FolderBack = path2.resolve(dirPathOut, '..', '..');
            const file = path2.join(FolderBack, 'src', 'features', fileNameEntities);
            ...
            const snippetfile = path2.join(FolderBack, 'snippets', "vhdl.json");
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Lade die Snippets aus der JSON-Datei
    const snippets = require('../../snippets/vhdl.json');

    // Registriere die Snippets für die VHDL-Sprache
    // Registriere die Snippets für die VHDL-Sprache
    context.subscriptions.push(
        vscode.languages.registerCompletionItemProvider('vhdl', {
            provideCompletionItems(document, position) {

                const snippetText = document.getText();
                const snippets = JSON.parse(snippetText);


                /*
                const completionItems = Object.keys(snippets).map(key => {
