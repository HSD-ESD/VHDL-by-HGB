//specific imports
import { VhdlParser } from './VhdlParser/VhdlParser';
...
export class HDLUtils {
    ...
    public static async GetDependencies(filePath: string): Promise<Set<string>> 
    {
        ...
        const EntityPackages = await VhdlParser.ParsePackages(filePath);
        ...
        //parse packages and add to dependencies
        const architecturePackages = await VhdlParser.ParsePackages(implementation[0].uri.fsPath);
        ...
        for (const symbol of instantiationSymbols) 
        {
            ...
            const instantiationPath : string = await VhdlParser.ParseInstantiation(doc, instantiationRange);
